...
class LabelCrossEntropy_LSR(nn.Module):
    def __init__(self):
        super(LabelCrossEntropy_LSR, self).__init__()

    def forward(self, outputs, targets):
        c = 0.05
        outputs_ls = F.log_softmax(outputs, dim=-1)
        batch_size, class_num = outputs_ls.shape
        loss_soft = -(outputs_ls * targets).sum() / batch_size
        u = torch.ones_like(o=utputs) / class_num
        #print('u', u)
        loss_u = -(outputs_ls * u).sum() / batch_size

        loss = (1 - c) * loss_soft + c * loss_u
        #H(q', p) = (1 - c) * H(q, p) + c * H(u, p)
...
class OnlineLabelSmoothing(nn.Module):
    def __init__(self, num_classes, device):
        ...
        #self.S = torch.eye(self.K).to(device) # case 2: one-hot
        #self.S = torch.zeros((self.K, self.K)).to(device) # case 3: label smooth
        #self.S[:, :] = self.smooth / self.K
        #self.S[torch.eye(self.K).byte()] = 1 - self.smooth + self.smooth / self.K
    ...
    def forward(self, outputs, targets):
        ...
        loss = self.alpha * self.CE(outputs, targets) + (1 - self.alpha) * self.CE(outputs, soft_targets)
        ...
        correct_prediction = outputs_prob[correct_index]
        ###################################################
        
        #correct_label = target_label[correct_index].tolist()
        #self.S_t[correct_label] += correct_prediction
        #self.count[correct_label] += 1
...
class Bootstrapping(nn.Module):
    #alpha=0:hard bootstrapping, alpha=1 soft bootstrapping
    def __init__(self, beta=0.95,alpha=0):
    ...
    def forward(self, outputs, targets):
        if self.alpha ==0:
            ...
            hard_targets = outputs_softmax.argmax(dim=1).view(-1,1)
            hard_targets_loss = outputs_softmax.gather(1,hard_targets)
            batch_size, _ = outputs.shape
            crossentropy = LabelCrossEntropy()
            hard_loss = self.beta*crossentropy(outputs, targets) - (1-self.beta)*hard_targets_loss.sum()/ batch_size
            return hard_loss
        else:
            #soft label
            crossentropy = LabelCrossEntropy()
            outputs_p = F.softmax(outputs.detach(), dim=1)
...
class MusicBERTM2PCriterionForXAI(SentencePredictionCriterion):
    def forward(self, model, sample, reduce=True):
        ...
        #print(targets)
        #print("***size***", logits.shape, targets.shape)
        ...
        #exit()
        acc = (preds == targets.argmax(dim=1)).sum()/len(preds)
        logging_output["ncorrect"] = (preds == targets.argmax(dim=1)).sum()
        print("===============================acc",acc)
        #exit()
